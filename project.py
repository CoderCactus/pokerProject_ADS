...
class Deck:
    ...
    def shuffle(self):
        ...
        '''
        for current in range(len(self.cards)):
            randSeed = random.randint(i, len(self.cards) - 1) # Pick a random card index
            self.cards[current], self.cards[randSeed] = self.cards[randSeed], self.cards[current] # Swaps the current card with the randomly chosen index card
